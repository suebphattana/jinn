...
### File Watcher
Uses `chokidar` to watch `~/.jimmy/` for changes and trigger appropriate reloads:
...
Stores session metadata (id, engine, employee, connector source, timestamps) in `jimmy.db`.
